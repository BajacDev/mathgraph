...
object Lexer
  ...
  def apply(source: AbstractSource)(ctxt: Context): Iterator[FrontendToken] = {
    ...
    println("Running TPTP lexer...")

    val lexerOutput = lexer(Source.fromIterator(source.source, SourcePositioner(source)))

    println("Filtering comments...")

    val filtered = lexerOutput.filter {
    ...
    println("TPTP parser is successful !\n")
    result
  }
}
