...
object Parser extends Parsers with Pipeline[Iterator[Token], TPTPProgram] {
  ...
  lazy val tptpFile: Syntax[TPTPProgram] = (many(tptpInput) ~ eof.skip).map { lines =>
  ...
  protected def apply(tokens: Iterator[Token])(ctxt: Context): TPTPProgram = {
