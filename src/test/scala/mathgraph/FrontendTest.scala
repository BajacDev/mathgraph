...
class ParserTest extends AnyFunSuite {
	object PrettyPrinter extends Pipeline[Program, String] {
      ...
      def rec(tree: Tree): String = tree match {
        case Program(lets, axioms) => (lets ++ axioms).map(rec).mkString("", ";\n", ";")
        case Let(name, Seq(), bodyOpt) => s"let $name" + bodyOpt.map(bd => " = " + rec(bd)).getOrElse("")
        case Let(name, vars, bodyOpt) => s"let $name(${vars.mkString(", ")})" + bodyOpt.map(bd => " = " + rec(bd)).getOrElse("")
	...
	def shouldSucceed(input: String, result: String): Unit = {
		try {
			...
			case err: FatalError => fail("Unexpected failure of the frontend: " + err, err)
		}
	}
    ...
    "forall x. has(x, y) -> in(y, x);" ~> Success("(forall x. (has(x, y) -> in(y, x)));")
	}
